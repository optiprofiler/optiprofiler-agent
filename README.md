...
> Top-down technical architecture (function-calling layout, the unified-agent tools, validators, persistence, and how to extend any of it) lives in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). A workflow-level map of what each user request calls, which tools run, and what each stage writes/returns lives in [`docs/AGENT_WORKFLOWS.md`](docs/AGENT_WORKFLOWS.md). Roadmap of upcoming work — including L4 constrained decoding, web search in the debugger path, and the long-term self-evolution loop — lives in [`docs/ROADMAP.md`](docs/ROADMAP.md).
